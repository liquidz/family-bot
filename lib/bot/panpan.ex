    ...
    cond do
      ...
      message.text =~ ~r/パンパン/ ->
        cond do
          ...
          message.text =~ ~r/(ひどい|嫌い)/ ->
            send_message("<@#{message.user}> :perry_sup:", message.channel, slack)
          message.text =~ ~r/(謝り|あやまり)なさい/ ->
            send_message("<@#{message.user}> ごめんなちゃい", message.channel, slack)
          true -> nil
        end
      true ->
        nil
